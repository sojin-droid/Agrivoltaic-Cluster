...
용인시 파일은 헤더가 "시도: 전체(시도), 시군구: 전체(시/군/구)"로 실제로는 전국 집계가
잘못 내려받아진 것으로 보여(용인시 단독 행 자체가 없음) 제외 — note로 표시.
"""
import glob
import json
import os
...
BROKEN_FILES = {"용인시"}  # 전국 데이터 오export로 확인, 재다운로드 대기
    ...
    for city in BROKEN_FILES:
        ...
        if code and code not in out["by_code"]:
            out["by_code"][code] = {
                ...
                "manuf_gwh_year": None,
                "source_files": [],
                "note": "원본 파일이 전국 집계로 잘못 내려받아져(용인시 단독 행 없음) 재다운로드 대기 중",
            }
